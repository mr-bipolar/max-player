...
import com.aaronmaxlab.maxplayer.MPVLib.MpvFormat.MPV_FORMAT_DOUBLE
import com.aaronmaxlab.maxplayer.MPVLib.MpvFormat.MPV_FORMAT_FLAG
import com.aaronmaxlab.maxplayer.MPVLib.MpvFormat.MPV_FORMAT_INT64
import com.aaronmaxlab.maxplayer.MPVLib.MpvFormat.MPV_FORMAT_NONE
import com.aaronmaxlab.maxplayer.MPVLib.MpvFormat.MPV_FORMAT_STRING
import java.io.File
...
internal class MPVView(context: Context, attrs: AttributeSet) : BaseMPVView(context, attrs) {
    ...
    override fun initOptions() {
        // Mimic default shared preferences
        ...
        val subFontFile = File(this.context.filesDir.path, "subfont.ttf")
        context.assets.open("NotoSansMalayalam-Regular.ttf").use { input ->
            subFontFile.outputStream().use { output ->
                input.copyTo(output)
            }
        }
    ...
    fun onKey(event: KeyEvent): Boolean {
        ...
        // Ignore repeated events (mpv handles repeat internally)
        ...
        if (mapped == null) {
            ...
            // Ignore dead keys (combining accents)
