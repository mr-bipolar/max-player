...
internal object Utils {
   ...
   private fun isTv(context: Context): Boolean {
    ...
    class PlaybackStateCache {
        ...
        private var playlistCountListener: ((Int) -> Unit)? = null

        fun setOnPlaylistCountChangedListener(listener: (Int) -> Unit) {
            playlistCountListener = listener
            // Notify immediately if already loaded
            if (playlistCount > 0) listener(playlistCount)
        }
...
//        }
            ...
            when (property) {
                ...
                "playlist-count" -> {
                    ...
                    Handler(Looper.getMainLooper()).post {
                        playlistCountListener?.invoke(playlistCount)
                    }
                }
                "time-pos" -> position = value * 1000
                else -> return false
    ...
    class OpenUrlDialog(
        context: Context,
        private val onOpen: (playlist: String, url: String) -> Unit,
        ...
        private val view = LayoutInflater.from(context)
            .inflate(R.layout.dialog_open_url, null)

        private val etPlaylist = view.findViewById<EditText>(R.id.etPlaylist)
        private val etUrl = view.findViewById<EditText>(R.id.etUrl)
        private val btnOpen = view.findViewById<Button>(R.id.btnOpen)
        private val headerLabel = view.findViewById<TextView>(R.id.header_label)
        ...
        init {
            if (isTv(context)) {
                TvFocusHelper.applyFocus(btnOpen)
            }
            // Setup UI based on mode
            if (isVideoMode) {
                etPlaylist.visibility = View.GONE
                headerLabel.text = headerText ?: context.getString(R.string.action_open_url)
            }

            btnOpen.isEnabled = false

            etUrl.addTextChangedListener { validateInputs() }
            etPlaylist.addTextChangedListener { validateInputs() }

            btnOpen.setOnClickListener {
                if (opened) return@setOnClickListener
                opened = true

                dialog.dismiss()
                onOpen(playlistName, url)
            }
        }

        private fun validateInputs() {
            val urlText = etUrl.text?.toString().orEmpty()
            val playlistText = etPlaylist.text?.toString().orEmpty()

            btnOpen.isEnabled = urlText.isNotEmpty() && validate(urlText)

            if (urlText.isNotEmpty() && !validate(urlText)) {
                etUrl.error = dialog.context.getString(R.string.uri_invalid_protocol)
            } else {
                etUrl.error = null
            }
        }
        ...
        val playlistName: String
            get() = etPlaylist.text.toString()

        val url: String
            get() = etUrl.text.toString()
    }
