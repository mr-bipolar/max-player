...
import com.aaronmaxlab.filepicker.AbstractFilePickerFragment
...
import com.aaronmaxlab.filepicker.FilePickerFragment
import com.aaronmaxlab.maxplayer.databinding.FragmentFilepickerChoiceBinding
import java.io.File
...
class FilePickerActivity : AppCompatActivity(), AbstractFilePickerFragment.OnFilePickedListener {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        when (intent.getIntExtra("skip", -1)) {
            ...
            DOC_PICKER -> {
                val root = intent.getStringExtra("root")!!.toUri()
                initDocPicker(root)
                return
            }
        }
    ...
    private fun getFilterState(): Boolean {
        return getDefaultSharedPreferencesCompat(this)
            .getBoolean("MainActivity_filter_state", false)
    }


    private fun saveFilterState(enabled: Boolean) {
        getDefaultSharedPreferencesCompat(this)
            .edit {
    ...
    private fun initFilePicker() {
        ...
        val sharedPrefs  = getDefaultSharedPreferencesCompat(this)
...
//    }
    ...
    private fun showUrlDialog() {
        Utils.OpenUrlDialog(
            this,
            onOpen = { _, url ->
                finishWithResult(RESULT_OK, url)
            },
            onCancel = {
