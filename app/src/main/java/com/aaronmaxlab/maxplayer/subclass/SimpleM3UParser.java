...
public class SimpleM3UParser {
    ...
    private final static String EXTINF_SUB_URL = "sub-url=\"";
    private final static String EXTINF_SUB_PREFIX = "sub-"; // sub-en, sub-ml
    // ########################
    // ##
    // ## Members
    // ##
    ...
    private M3U_Entry parseExtInf(String line) {
        ...
        while (!line.isEmpty() && !line.startsWith(",") && !line.equals(old)) {
            ...
            if (line.startsWith(EXTINF_TVG_NAME) && line.length() > EXTINF_TVG_NAME.length()) {
                ...
                line = line.substring(i + 1);
            } else if (line.startsWith(EXTINF_SUB_URL)) {
            line = line.substring(EXTINF_SUB_URL.length());
            int i = line.indexOf("\"");
            curEntry.subtitleUrl = line.substring(0, i);
            line = line.substring(i + 1);

        } else if (line.startsWith(EXTINF_SUB_PREFIX)) {
            // sub-en="...", sub-ml="..."
            int langStart = EXTINF_SUB_PREFIX.length();
            int eq = line.indexOf("=\"");
            if (eq > langStart) {
                String lang = line.substring(langStart, eq);
                line = line.substring(eq + 2);
                int i = line.indexOf("\"");
                String subUrl = line.substring(0, i);
                curEntry.subtitles.put(lang, subUrl);
                line = line.substring(i + 1);
            } else {
                line = line.substring(1);
            }
    ...
    public static class M3U_Entry {
        ...
        public String subtitleUrl;  // default subtitle
        public Map<String, String> subtitles = new HashMap<>(); // lang → url
